...
//        parent::_before_update($data, $options); // TODO: Change the autogenerated stub
        ...
        if($_FILES['logo']['error'] === 0){
            ...
            $upload->exts = C('IMG_exts');  // 设置附上传类型
            //$upload->rootPath = $rotPath; //设置附件上传根目录
            $upload->savePath = 'Goods/'; //图片二级目录的名称
            //上传文件
            $info = $upload->upload();
            if(!$info)
            {
                //先把上传失败的错误信息存到模型中，由控制器最终获取这个错误信息并且显示
                $this->error = $upload->getError();
                return FALSE; // 返回控制器
            }
            else{
    ...
    public  function search()
    {
        ...
        elseif ($startAddtime)
            $where['addtime'] = array('egt',strtotime("startAddtime 00:00:001"));
        elseif ($endAddtime)
