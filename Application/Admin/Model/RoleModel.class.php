...
class RoleModel extends Model 
{
	...
	protected $_validate = array(
		...
		array('role_name', '1,30', '角色名称已存在！', 1, 'unique', 3),
	);
	public function search($pageSize = 20)
	{
		 ...
		 *=c.id GROUP BY a.id;
		*/
		$data['data'] = $this->field('a.*,GROUP_CONCAT(c.pri_name) pri_name')->join('LEFT JOIN php34_role_privilege b ON a.id = 
		b.role_id LEFT JOIN php34_privilege c ON b.pri_id=c.id')->alias('a')->where($where)->group('a.id')->limit($page->firstRow.','.$page->listRows)->select();
		return $data;
	}
	// 添加前
