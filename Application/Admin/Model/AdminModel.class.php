...
{
	protected $insertFields = array('username','password','cpassword','chkcode','is_use');
...
//		$data['data'] = $this->alias('a')->where($where)->group('a.id')->limit($page->firstRow.','.$page->listRows)->select();
        $data['data'] = $this->field('a.*,GROUP_CONCAT(c.role_name) role_name')->join('LEFT JOIN php34_admin_role b ON a.id = 
		b.admin_id LEFT JOIN php34_role c ON b.role_id=c.id')->alias('a')
	...
	protected function _before_delete($option)
	{
        ...
        //删除admin表管理员信息之前先删除admin_role表中这个管理员对于数据
		$arModel = M('AdminRole');
        $arModel->where(array('admin_id'=>array('eq',$option['where']['id'])))->delete();
	}
